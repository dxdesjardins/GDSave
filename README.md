...
2. Each saveable scene must have a `Saver` node exposed in the editor for serialization. The Godot Engine has no easy way to declare non-root nested scene nodes as serializable. This means that nested saveable scenes must either the root node be a `Saver` or you must set `Editable Children` as true for all nested saveable scenes. View a pull request that could fix this issue: https://github.com/godotengine/godot/pull/84018.
